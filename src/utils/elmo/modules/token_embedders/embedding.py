...
from elmo.common.file_utils import get_file_extension, cached_path
from elmo.data import Vocabulary
from elmo.modules.token_embedders.token_embedder import TokenEmbedder
from elmo.modules.time_distributed import TimeDistributed
...
def _read_embeddings_from_text_file(file_uri: str,
        ...
        for line in embeddings_file:
            ...
            if token in tokens_to_keep:
                ...
                if len(fields) - 1 != embedding_dim:
                    ...
                    warn("Found line with wrong number of dimensions (expected: %d; actual: %d): %s",
    ...
    for i in range(vocab_size):
        ...
        if token in embeddings:
            ...
            info("Token %s was not found in the embedding file. Initialising randomly.", token)
...
class EmbeddingsTextFile(Iterator[str]):
    ...
    def __init__(self,
        ...
        if zipfile.is_zipfile(main_file_local_path):  # ZIP archive
            ...
            if package is None:
                warn('The embeddings file has an unknown file extension "%s". '
    ...
    def _get_num_tokens_from_first_line(line: str) -> Optional[int]:
        ...
        if 1 <= len(fields) <= 2:
            try:
                ...
                info('Recognized a header line in the embedding file with number of tokens: %d',
                     num_tokens)
                return num_tokens
        return None
